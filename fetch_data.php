         <?php
			...
			$con = new PDO("mysql:host=localhost;dbname=tsoft", 'root', '');

			if (isset($_POST["submit"])) {
				$str = $_POST["search"];
				$sth = $con->prepare("SELECT * FROM `products` WHERE product_brand = '$str'");
         ...
         <?php
			...
			if (isset($_POST["action"])) {
				$query = "
		SELECT * FROM products WHERE product_status = 'Active' AND product_id BETWEEN 0 AND 100 
	";
